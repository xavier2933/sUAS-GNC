...
%   1. AGENT_FILE (set below) — explicit override, e.g. 'saved_agents/sac_.../Agent340.mat'
%   2. saved_agents/sac_*/AgentN.mat — highest-numbered SAC checkpoint (auto-detected)
%   3. sac_slg_agent.mat             — end-of-run save from TrainSAC.m
%
% SAC inference note:
%   getAction() on an rlSACAgent returns a SAMPLE from the Gaussian policy
%   (same as during training). For smoother deterministic rollouts you can
%   extract the policy mean directly from the actor network — see comment
%   near the rollout loop below.
...
%% ─────────────────────────────────────────────
AGENT_FILE = 'saved_agents/sac_0420_1916/Agent897.mat';  % ← update to your best checkpoint 356, 913 avg 4205
...
rl_obs    = zeros(6,  MaxSteps+1);  rl_obs(:,1)    = obs0;
...
while ~isDone && k < MaxSteps
    ...
    % For a deterministic (mean-only) rollout, replace with:
    %   actorNet = getActor(agent).getLearnableParameters;  % not straightforward
    %   -- simplest: just use getAction; SAC's policy std is low once trained.
...
C_rl  = [0.13 0.63 0.30];   % green (SAC — different from PPO red for easy comparison)
