...
% Created: 4/8/23
%  
% This is a helper that students can use to complete HW 10. 
%
%
...
for i=1:n_ind
    ...
    wind_body_est = utils.TransformFromInertialToBody(wind_inertial_est(:,i), aircraft_state_est(4:6,i));
    air_rel_est = aircraft_state_est(7:9,i) - wind_body_est;
    wind_angles_est(:,i) = utils.AirRelativeVelocityVectorToWindAngles(air_rel_est);
    ...
    %%% STUDENTS REPLACE WITH YOUR GUIDANCE ALGORITHM FROM HOMEWORK 7 <=======================
    control_objectives = hw7utils.OrbitGuidance(aircraft_state_con(1:3,i), gvf_speed, gvf_radius, gvf_center, gvf_flag, gvf_gains);
    ...
    if (CONTROL_FLAG==FEED)
        [control_out, x_c_out] = hw7utils.SLCWithFeedForwardAutopilot(Ts*(i-1), aircraft_state_con(:,i), wind_angles_con(:,i), control_objectives, control_gain_struct);
    else
        [control_out, x_c_out] = frewhw6utils.SimpleSLCAutopilot(Ts*(i-1), aircraft_state_con(:,i), wind_angles_con(:,i), control_objectives, control_gain_struct);
    end
...
frewhw6utils.PlotSimulationWithCommands(time_iter,aircraft_state_est,control_array, wind_inertial_est, x_command, 'r--')
